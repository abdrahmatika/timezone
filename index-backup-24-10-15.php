...
// 2010 ke bawah
	...
	if ( isset($_SESSION['filter']) ) {
		...
		$query = "SELECT TAHUN, SMSTER, NIM, SMSTR FROM t_krs_sementara_lama WHERE SMSTER != '' $filter_thn $filter_prodi $filter_sms $filter_angkatan $cari GROUP BY NIM";

		$query_jml = "SELECT TAHUN, SMSTER, NIM, SMSTR FROM t_krs_sementara_lama WHERE SMSTER != '' 
...
// TAHUN < 2011 
							...
							// total bobot & sks semester terakhir
							$sql_bobot = $conn->query("SELECT * FROM t_krs_sementara_lama 
							...
							$sql2 = $conn->query("SELECT * FROM t_krs_sementara_lama 
															WHERE NIM = '$r[NIM]'  AND TAHUN <= '$_SESSION[filter_tahun]' AND SMSTR != '0' AND SMSTR <= '$r[SMSTR]'");
							...
							$tot_bobot_all 	= $tot_bobot_pdb;
							$tot_sks_all  	= $tot_sks_pdb;
